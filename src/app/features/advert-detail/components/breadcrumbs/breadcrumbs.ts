...
export class Breadcrumbs {
    ...
    breadcrumbs = toSignal<BreadcrumbItem[]>(
        this.categoriesBusiness.getAll().pipe(
            switchMap(categories => this.buildBreadcrumb(categories, this.categoryId()))
        )
    );
    private buildBreadcrumb(categories: Category[], id: string): Observable<BreadcrumbItem[]> {
